...
namespace WindowsFormsApp1
{
    public partial class Form1 : Form
    {
        ...
        int pX = 200, pY = 500, pSize = 100, pHp = 100, pDmg = 20;
        int eX = 100, eY = 100, eSize = 100, eHp = 100, eDmg = 25, eSpeed = 3, eDir = 1;
        public Form1()
        {
            InitializeComponent();
            dc = CreateGraphics();
            ships[0] = new Player(pX, pY, pSize, pHp, eDmg, 0, 0);
            ships[1] = new Enemy(eX, eY, eSize, eHp, pDmg, eSpeed, 0);
            Random r = new Random();
            for (int i = 2; i < scount; i++)
            {
                int enbrndX = r.Next(40, 800);
                ships[i] = new EnemyNoBrain(enbrndX, eY, eSize, eHp, pDmg, eSpeed, eDir);
            }
            for(int i = 1; i< scount; i++)
                if(ships[i]!= null)
                    healthLines[i] = new HealthLine(ships[i].X, ships[i].Y, ships[i].Size, ships[i].Hp, ships[i].Hp);
        ...
        private void timer1_Tick(object sender, EventArgs e)
        {
            ...
            for(int i = 1;i<scount;i++)
                if (ships[i]!=null)
                    healthLines[i].LineSize(ships[i].X - 50, ships[i].Y - 65, ships[i].Size, ships[i].Hp, eHp);
